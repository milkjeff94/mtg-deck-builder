...
Usage (3 terminals recommended):
  # Terminal A: start the mock feed (writes to repo logs/Player.log by default)
  python3 tools/mock_draft_feed.py --interval 25
...
Fast mode:
  python3 tools/mock_draft_feed.py --interval 5 --picks 12
"""
...
def default_log_path() -> Path:
  # Default to repo-local logs/Player.log
  return Path(__file__).resolve().parents[1] / "logs" / "Player.log"


def main():
  ap = argparse.ArgumentParser()
  ap.add_argument("--out", default=str(default_log_path()), help="Path to mock Player.log (default: repo logs/Player.log)")
  ap.add_argument("--interval", type=float, default=25.0, help="Seconds between picks (offer→pick→next)")
